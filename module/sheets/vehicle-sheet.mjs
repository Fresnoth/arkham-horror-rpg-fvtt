...
export class ArkhamHorrorVehicleSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
    #dragDrop // Private field to hold dragDrop handlers

    /** @inheritDoc */
    static DEFAULT_OPTIONS = {
        classes: ['sheet', 'actor', 'vehicle'],
        tag: 'form',
        position: {
            width: 700,
            height: 800
    ...
    static PARTS = {
        header: {
            id: 'header',
            template: 'systems/arkham-horror-rpg-fvtt/templates/vehicle/parts/vehicle-header.hbs'
        },
        tabs: {
            id: 'tabs',
        ...
        vehicle: {
            id: 'vehicle',
            template: 'systems/arkham-horror-rpg-fvtt/templates/vehicle/parts/vehicle-main.hbs',
            scrollable: ['']
        },
        biography: {
            id: 'biography',
 ...
 */
    ...
    _prepareItems() {
        ...
        for (let i of inventory) {
            if (i.type === 'useful_item' || i.type === 'protective_equipment' || i.type === 'weapon' || i.type === 'relic' || i.type === 'tome') {
                cargo.push(i);
                if(game.settings.get("arkham-horror-rpg-fvtt", "characterLoadCapacity")){
                    if(i.system.weight > 0){
                        totalWeight += i.system.weight * (i.system.quantity || 1);
                    }
                }
            }
        }
        ...
        // sort items by name
        cargo = cargo.sort((a, b) => a.name.localeCompare(b.name));
